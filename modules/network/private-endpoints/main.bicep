...
param numsuffix string = '01'

var lockName = toLower('${privateEndpoint.name}-${resourcelock}-lck')

var privateEndpointName = toLower('${targetResourceName}-${toLower(targetSubResourceType)}-pep${numsuffix}')
...
module nicInfo 'nicInfo.bicep' = {
  name: 'nicInfo-${privateEndpoint.name}-${uniqueString(privateEndpoint.name, resourceGroup().name)}'
  params: {
